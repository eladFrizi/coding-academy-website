...
(function putChaptchaCallback() {
	...
	// 	'handleCaptchaSuccess';
		$('.g-recaptcha').attr('data-callback','handleCaptchaSuccess')
})();

function handleCaptchaSuccess() {
	var elCaptcha = document.querySelector('.g-recaptcha > div > div > iframe');
	console.log({ elCaptcha: elCaptcha });
	$(elCaptcha).attr('style', '');
}
...
(function($) {
	...
	SEMICOLON.mySlider = {
		...
		sliderParallaxDimensions: function() {
			if ($mySliderParallaxEl.find('.slider-parallax-inner').length < 1) {
				return true;
			}
			...
			if (
				...
				if (
					$mySliderParallaxEl.hasClass('revslider-wrap') ||
					$mySliderParallaxEl.find('.carousel-widget').length > 0
				) {
					parallaxElHeight = $mySliderParallaxEl
						.find('.slider-parallax-inner')
						.children()
						.first()
						.outerHeight();
				...
				element.find('form').validate({
					submitHandler: function(form) {
						if (!grecaptcha.getResponse()) {
							var elCaptcha = document.querySelector(
								'.g-recaptcha > div > div > iframe'
							);
							$(elCaptcha).attr(
								'style',
								'border: 3px solid red !important; border-radius: 3px'
							);
							return false;
						}
						elementResult.hide();
							...
							success: function(data) {
								if (data.alert === 'success') {
									...
									window.location.assign(
										'http://coding-academy.org/thank.html'
									);
								}
